...
> 📎 **併存する実戦プレイブック**: この run には学術研究マップ（本ファイル + `cluster-01〜07` + `gather/` + `retrieval/`）に加えて、**実戦寄りの攻略プレイブック**（[`strategy-index.md`](strategy-index.md) + `strategy-01〜09`）が併存します。**コンペ仕様の実取得データ・実エンジンAPI・メタデッキ・操縦の鉄則・行動計画**は `strategy-*` を、**先行手法の学術的背景**は本マップを参照してください。
