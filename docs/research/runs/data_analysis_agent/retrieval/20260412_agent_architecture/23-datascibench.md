...
このフレームワークにより、多様な出力形式を統一的なブール値（0/1）または小数値（0-1）のメトリクスに変換する。

**評価メトリクス体系**:

粗粒度メトリクス:
- **Success Rate (SR)**: 10回実行での完全成功率
- **Completion Rate (CR)**: ステップレベルのスコアリング（各ステップ0-2点）

細粒度メトリクス（5つの代表的集約関数）:
- **F1**: Data Quality Score（クリーニングタスク）
- **F2**: Plot Validity（可視化精度）
- **F3**: Data Accuracy（探索/統計）
- **F4**: Visualization Completeness（ファイル存在確認）
- **F5**: Model Accuracy（予測タスク）

追加評価:
- **VLM-as-a-judge**: GPT-4o-miniによる視覚品質評価
...
3. **グラウンドトゥルース生成**: マルチサンプリングによる一貫性検証、非BigCodeBenchプロンプトは3名の著者の合意による相互検証

## アルゴリズム（擬似コード）

```
Algorithm: DataSciBench TFC Evaluation Framework
Input: モデル出力 O, タスクタイプ T, グラウンドトゥルース GT
Output: 評価スコア S ∈ [0, 1]
...
35: procedure CONSTRUCT_BENCHMARK()
36:   // データセット構築パイプライン
37:   P_raw ← COLLECT(CodeGeeX, BigCodeBench, Human, LLM)
38:   P_filtered ← KEYWORD_FILTER(P_raw, DS_keywords)
39:   P_reviewed ← EXPERT_REVIEW_3STAGE(P_filtered)
40:
41:   // グラウンドトゥルース生成
42:   for each prompt p in P_reviewed do
43:     samples ← MULTI_SAMPLE(LLM, p, n=k)
44:     if SELF_CONSISTENT(samples) then
45:       GT[p] ← MAJORITY_VOTE(samples)
46:     else
47:       GT[p] ← HUMAN_CONSENSUS(p, reviewers=3)
48:     end if
49:   end for
50:
51:   // TFC設定
52:   for each (p, gt) in (P_reviewed, GT) do
53:     T[p] ← CLASSIFY_TASK_TYPE(p)
54:     F[p] ← SELECT_AGG_FUNCTIONS(T[p])
55:     C[p] ← GENERATE_EVAL_CODE(F[p], gt)
56:   end for
57:   return {(p, T[p], F[p], C[p], GT[p])}
58: end procedure
```
...
| ベンチマーク | 評価方式 | タスク複雑度 | GT生成 | プロンプト数 |
|---|---|---|---|---|
| HumanEval | テストケース | 単一関数 | 手動 | 164 |
| MBPP | テストケース | 単一関数 | 手動 | 974 |
| BigCodeBench | テストケース | マルチステップ | 手動 | 1,140 |
| **DataSciBench** | **集約メトリクス + プログラム的ルール** | **マルチステップ** | **半自動** | **222** |

DataSciBenchは「集約メトリクスとプログラム的ルール」を使用する点で既存ベンチマークと差別化される。

### Table 2: 全体結果（全23モデル）

| モデル | カテゴリ | Final Score (%) | SR (%) | CR (%) |
|---|---|---|---|---|
| GPT-4o-2024-05-13 | API | 64.51 | 66.31 | 68.44 |
| Deepseek-Coder-33B | コード特化OS | 56.76 | 55.86 | 61.23 |
| GPT-4-Turbo | API | 54.65 | 51.93 | 58.87 |
| GPT-4o-mini | API | 54.18 | 50.63 | 57.78 |
| Claude-3.5-Sonnet | API | 52.29 | - | - |
| Qwen2.5-Coder-7B | コード特化OS | 47.67 | 45.18 | 53.11 |
| Qwen2.5-7B | 汎用OS | 45.99 | - | - |
| Deepseek-Coder-6.7B | コード特化OS | 38.45 | - | - |
| GLM-4-Flash | API | 30.74 | - | - |
| o1-mini | API | 29.77 | - | - |
| CodeLlama-13B | コード特化OS | 12.64 | - | - |
...
| 比較対象 | ピアソン相関 | スピアマン相関 |
|---|---|---|
| LiveCodeBench | 0.853 | 0.673 |
| BigCodeBench | 0.823 | 0.808 |

### Figure（追加）: 難易度別性能分布
Easy/Medium/Hardの3難易度レベルにわたるモデル性能を可視化。トップモデル（GPT-4o、GPT-4-Turbo、Deepseek-Coder-33B）は難易度にわたって一貫した性能を維持するが、小規模モデルは難易度上昇に伴い劇的な性能低下を示す。
...
- **難易度レベル**: Easy（167件）、Medium（30件）、Hard（25件）
- **実行回数**: Success Rate算出のため各プロンプト10回実行
- **評価メトリクス**: 粗粒度（CR, SR）+ 細粒度（VLM, F1-F5）の重み付け組み合わせ
...
**大規模モデルの逆転現象**:
- CodeLlama-34B（1.33%）がCodeLlama-13B（12.64%）を大幅に下回る異常事態
- 訓練データのフォーマットバイアスにより、指示追従の柔軟性が制限されたことが原因

**難易度別分析**:
- トップモデル（GPT-4o、GPT-4-Turbo、Deepseek-Coder-33B）は難易度にわたって一貫
- 小規模モデルはHardタスクで劇的な性能低下

**ベンチマーク間の相関**:
- LiveCodeBenchとのピアソン相関: 0.853、BigCodeBenchとの相関: 0.823
...
- CodeLlama-34Bの逆転現象は、モデルスケーリングだけではタスク準拠を保証できないことを示す
- データ分析エージェントの設計において、TFCのような構造化された評価フレームワークは品質保証メカニズムとして直接応用可能
- 可視化タスク評価とVLM-as-a-judge手法の精度限界が著者らにより認められており、批評モデルの訓練による改善が提案されている
- コードとデータはGitHub（THUDM/DataSciBench）で公開
