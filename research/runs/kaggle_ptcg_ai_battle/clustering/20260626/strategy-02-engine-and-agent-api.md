...
> ✅ 以下は要点抜粋。**全49種（0–48）の公式定義・正確な整数値は [`strategy-09`](strategy-09-PRIMARY-official-data-api.md)（公式 `cg/api.py` 由来の一次情報）を参照**。特に `ATTACK=35`（SelectContext側）/ `EVOLVE=37` / `COIN_HEAD=46` 等は strategy-09 で確定。
...
def hand_score(id):
    ...
    ...
```
各分岐の数値は**トップ人間プレイヤーの棋譜（divergence分析）から校正**する（後述strategy-06）。
