...
1. Identify the **domain** (`<domain>`, `snake_case`):
   - Infer from input file path if it lives under `research/(runs|domains)/<domain>/...`
   - Otherwise infer from the research theme or ask the user
2. If `research/domains/<domain>/domain.yaml` exists and defines `output_paths.clustering`, use it.
3. Otherwise use the default path:

   ```
   research/runs/<domain>/clustering/<YYYYMMDD>/
   ```
   ...
   - clustering は日付ディレクトリ単位で、クラスタサブディレクトリは持ちません（gather / retrieval のみ `<YYYYMMDD>/<cluster>/`）。
4. **Never write directly under `research/domains/<domain>/`** — that layer is composed of symlinks pointing into `runs/`.
