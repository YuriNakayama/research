...
>
> 🔴 **[strategy-08で訂正] このファイルのシェア/勝率は単一の参考実装（GitHub `wmh/ptcg-abc`）の自己記録であり、敵対的検証で一部が裏取り不可だった**: 「Crustle 50%」「Dragaplutメタ首位」等の具体シェアは未確証。**外部サイト（LimitlessTCG）の相性数値は人間トーナメント由来で、cabtシミュレーション環境とは母集団が異なる**ため転用不可。本ファイルの数値は「考え方のテンプレート」として読み、**コンペ用の真のメタ・相性は日次エピソード(~720MB)の自前解析でのみ得られる**。詳細は [`strategy-08`](strategy-08-deep-research-verified.md)。
